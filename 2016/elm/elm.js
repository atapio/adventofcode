...
Elm.Light = Elm.Light || {};
Elm.Light.make = function (_elm) {
   "use strict";
   _elm.Light = _elm.Light || {};
   if (_elm.Light.values) return _elm.Light.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var update = F2(function (action,state) {
      ...
      {case "TurnOn": return true;
         ...
         default: return _U.eq($Basics.not(state),false);}
   });
   var Toggle = {ctor: "Toggle"};
   var TurnOff = {ctor: "TurnOff"};
   var TurnOn = {ctor: "TurnOn"};
   var init = false;
   return _elm.Light.values = {_op: _op
                              ,init: init
                              ,update: update
                              ,view: view
                              ,TurnOn: TurnOn
...
Elm.LightGrid = Elm.LightGrid || {};
Elm.LightGrid.make = function (_elm) {
   "use strict";
   _elm.LightGrid = _elm.LightGrid || {};
   if (_elm.LightGrid.values) return _elm.LightGrid.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   $Light = Elm.Light.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var pointsBetween = F3(function (gridSize,from,to) {
      return _U.eq(to.y,from.y) ? A2($List.map,
      function (x) {
         return _U.update(from,{x: x});
      },
      _U.range(from.x,to.x)) : A2($List.append,
      A3(pointsBetween,gridSize,_U.update(from,{y: to.y}),to),
      A3(pointsBetween,gridSize,from,_U.update(to,{y: to.y - 1})));
   });
   var pointIndex = F2(function (gridSize,point) {
      return gridSize.x * point.y + point.x;
   });
   var updateLight = F2(function (action,l) {
      return $Maybe.Just(A2($Light.update,
      action,
      A2($Maybe.withDefault,$Light.init,l)));
   });
   var update = F2(function (action,state) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "TurnOn": var points = A2($List.map,
           function (n) {
              return A2(pointIndex,state.size,n);
           },
           A3(pointsBetween,state.size,_p0._0,_p0._1));
           return _U.update(state,
           {lights: A3($List.foldr,
           F2(function (l,d) {
              return A3($Dict.update,l,updateLight($Light.TurnOn),d);
           }),
           state.lights,
           points)});
         case "TurnOff": var points = A2($List.map,
           function (n) {
              return A2(pointIndex,state.size,n);
           },
           A3(pointsBetween,state.size,_p0._0,_p0._1));
           ...
           {lights: A3($List.foldr,
           F2(function (l,d) {
              return A3($Dict.update,l,updateLight($Light.TurnOff),d);
           }),
           state.lights,
           points)});
         default: var points = A2($List.map,
           function (n) {
              return A2(pointIndex,state.size,n);
           },
           A3(pointsBetween,state.size,_p0._0,_p0._1));
           ...
           {lights: A3($List.foldr,
           F2(function (l,d) {
              return A3($Dict.update,l,updateLight($Light.Toggle),d);
           }),
           state.lights,
           points)});}
   });
   ...
   var TurnOff = F2(function (a,b) {
      return {ctor: "TurnOff",_0: a,_1: b};
   });
   var TurnOn = F2(function (a,b) {
      return {ctor: "TurnOn",_0: a,_1: b};
   });
   var lightsOn = function (grid) {
      return $List.length($Dict.keys(A2($Dict.filter,
      F2(function (k,v) {    return _U.eq(v,true);}),
      grid.lights)));
   };
   var init = F2(function (width,height) {
      return {size: {x: width,y: height},lights: $Dict.empty};
   });
   var Model = F2(function (a,b) {
      return {size: a,lights: b};
   });
   var Point = F2(function (a,b) {    return {x: a,y: b};});
   return _elm.LightGrid.values = {_op: _op
                                  ,Point: Point
                                  ,Model: Model
                                  ,init: init
                                  ,lightsOn: lightsOn
...
Elm.Main.make = function (_elm) {
   ...
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Light = Elm.Light.make(_elm),
   $LightGrid = Elm.LightGrid.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var lightGridSuite = A2($ElmTest.suite,
   "LightGrid",
   _U.list([A2($ElmTest.test,
           "Point Index is calculated correctly",
           A2($ElmTest.assertEqual,
           3,
           A2($LightGrid.pointIndex,{x: 4,y: 4},{x: 3,y: 0})))
           ,A2($ElmTest.test,
           "Point Index is calculated correctly",
           A2($ElmTest.assertEqual,
           5050,
           A2($LightGrid.pointIndex,{x: 100,y: 100},{x: 50,y: 50})))
           ,A2($ElmTest.test,
           "Points between is calculated correctly 1",
           A2($ElmTest.assertEqual,
           1000000,
           $List.length(A3($LightGrid.pointsBetween,
           {x: 1000,y: 1000},
           {x: 0,y: 0},
           {x: 999,y: 999}))))
           ,A2($ElmTest.test,
           "Points between is calculated correctly 2",
           A2($ElmTest.assertEqual,
           1000,
           $List.length(A3($LightGrid.pointsBetween,
           {x: 1000,y: 1000},
           ...
           {x: 999,y: 0}))))
           ,A2($ElmTest.test,
           "Points between is calculated correctly 3",
           A2($ElmTest.assertEqual,
           4,
           $List.length(A3($LightGrid.pointsBetween,
           {x: 1000,y: 1000},
           ...
           {x: 500,y: 500}))))
           ,A2($ElmTest.test,
           "Lights On is calculated correctly",
           A2($ElmTest.assertEqual,
           0,
           $LightGrid.lightsOn(A2($LightGrid.init,100,100))))
           ,A2($ElmTest.test,
           "LightGrid updates properly",
           A2($ElmTest.assertEqual,
           4,
           $LightGrid.lightsOn(A2($LightGrid.update,
           A2($LightGrid.TurnOn,{x: 499,y: 499},{x: 500,y: 500}),
           A2($LightGrid.init,100,100)))))
           ,A2($ElmTest.test,
           "LightGrid updates properly",
           A2($ElmTest.assertEqual,
           1000,
           $LightGrid.lightsOn(A2($LightGrid.update,
           A2($LightGrid.TurnOn,{x: 0,y: 0},{x: 999,y: 0}),
           A2($LightGrid.init,100,100)))))]));
   var lightSuite = function () {
      var onLight = $Light.init;
      var offLight = A2($Light.update,$Light.TurnOn,$Light.init);
      return A2($ElmTest.suite,
      "Light",
      _U.list([A2($ElmTest.test,
              "Light is initialized off",
              A2($ElmTest.assertEqual,false,$Light.init))
              ,A2($ElmTest.test,
              "Light can be turned on",
              A2($ElmTest.assertEqual,
              true,
              A2($Light.update,$Light.TurnOn,offLight)))
              ,A2($ElmTest.test,
              "Light can be turned off",
              A2($ElmTest.assertEqual,
              false,
              A2($Light.update,$Light.TurnOff,onLight)))
              ,A2($ElmTest.test,
              "Light can be toggled",
              A2($ElmTest.assertEqual,
              false,
              A2($Light.update,$Light.Toggle,onLight)))
              ,A2($ElmTest.test,
              "Light can be toggled",
              A2($ElmTest.assertEqual,
              true,
              A2($Light.update,$Light.Toggle,offLight)))]));
   }();
   var tests = A2($ElmTest.suite,
   "All",
   _U.list([lightSuite,lightGridSuite]));
   var main = $ElmTest.elementRunner(tests);
   return _elm.Main.values = {_op: _op
                             ,tests: tests
                             ,lightSuite: lightSuite
                             ,lightGridSuite: lightGridSuite
                             ,main: main};
};
